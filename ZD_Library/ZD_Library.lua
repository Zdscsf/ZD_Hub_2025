...
local requestsDisabled = true --getgenv and getgenv().DISABLE_ZDLibrary_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.672"
local ZDLibraryFolder = "ZDLibrary"
local ConfigurationFolder = ZDLibraryFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		ZDLibraryOpen = {Type = 'bind', Value = 'K', Name = 'ZDLibrary Keybind'},
		-- buildwarnings
		-- ZDLibraryprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(ZDLibraryFolder) then
				if isfile and isfile(ZDLibraryFolder..'/settings'..ConfigurationExtension) then
					file = readfile(ZDLibraryFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"ZDLibraryOpen":{"Value":"K","Type":"bind","Name":"ZDLibrary Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"ZDLibrary Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('ZDLibrary had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					...
					warn("Failed to load or execute the reporter. \nPlease notify ZDLibrary developers at discord.")
				end
			end)
...
repeat
	if ZDLibrary:FindFirstChild('Build') and ZDLibrary.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('ZDLibrary | Build Mismatch')
		print('ZDLibrary may encounter issues as you are running an incompatible interface version ('.. ((ZDLibrary:FindFirstChild('Build') and ZDLibrary.Build.Value) or 'No Build') ..').\n\nThis version of ZDLibrary is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end
...
if gethui then
	ZDLibrary.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(ZDLibrary)
	ZDLibrary.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	ZDLibrary.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == ZDLibrary.Name and Interface ~= ZDLibrary then
			Interface.Enabled = false
			Interface.Name = "ZDLibrary-Old"
		end
	end
elseif not useStudio then
...
if ZDLibrary.AbsoluteSize.X < minSize.X and ZDLibrary.AbsoluteSize.Y < minSize.Y then
...
local Main = ZDLibrary.Main
local MPrompt = ZDLibrary:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = ZDLibrary:FindFirstChild('Drag')
...
ZDLibrary.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/Zdscsf/ZD_Hub_2025/refs/heads/main/Other/icons.lua')
-- Variables
...
local Notifications = ZDLibrary.Notifications

local SelectedTheme = ZDLibraryLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = ZDLibraryLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	ZDLibrary.Main.BackgroundColor3 = SelectedTheme.Background
	ZDLibrary.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	ZDLibrary.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	ZDLibrary.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	ZDLibrary.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	ZDLibrary.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	ZDLibrary.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		ZDLibrary.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		ZDLibrary.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(ZDLibrary:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("ZDLibrary | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("ZDLibrary | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('ZDLibrary had an issue decoding the configuration file, please try delete the file and reopen ZDLibrary.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(ZDLibraryLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("ZDLibrary | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--ZDLibraryLibrary:Notify({Title = "ZDLibrary Flags", Content = "ZDLibrary was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(ZDLibraryLibrary.Flags) do
...
function ZDLibraryLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('ZDLibrary | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			ZDLibraryLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show ZDLibrary'.", Duration = 7, Image = 4400697855})
		else
			ZDLibraryLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.ZDLibraryOpen.Value or 'P'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(ZDLibraryFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('ZDLibrary Settings', 0, true)

	if TabList['ZDLibrary Settings'] then
		TabList['ZDLibrary Settings'].LayoutOrder = 1000
	end

	if Elements['ZDLibrary Settings'] then
		Elements['ZDLibrary Settings'].LayoutOrder = 1000
	end
...
function ZDLibraryLibrary:CreateWindow(Settings)
	if ZDLibrary:FindFirstChild('Loading') then
		if getgenv and not getgenv().ZDLibraryCached then
			ZDLibrary.Enabled = true
			ZDLibrary.Loading.Visible = true

			task.wait(1.4)
			ZDLibrary.Loading.Visible = false
		end
	end

	if getgenv then getgenv().ZDLibraryCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				ZDLibraryLibrary:Notify({Title = 'Build Mismatch', Content = 'ZDLibrary may encounter issues as you are running an incompatible interface version ('.. ((ZDLibrary:FindFirstChild('Build') and ZDLibrary.Build.Value) or 'No Build') ..').\n\nThis version of ZDLibrary is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(ZDLibraryFolder) then
		makefolder(ZDLibraryFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "ZDLibrary"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "ZDLibrary Interface Suite" then
		LoadingFrame.Version.Text = "ZDLibrary UI"
	end
	...
	if not Settings.DisableZDLibraryPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				ZDLibraryLibrary:Notify({
					Title = "ZDLibrary Interface",
					Content = "Enjoying this UI library? Find it at discord",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(ZDLibraryFolder.."/Discord Invites") then
			makefolder(ZDLibraryFolder.."/Discord Invites")
		end

		if isfile and not isfile(ZDLibraryFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(ZDLibraryFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"ZDLibrary RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(ZDLibraryFolder.."/Key System") then
			makefolder(ZDLibraryFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("ZDLibrary | "..Key.." Error " ..tostring(Response))
					warn('Check discord for help with ZDLibrary specific development.')
				end
			end
		end
		...
		if isfile and isfile(ZDLibraryFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(ZDLibraryFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(ZDLibraryFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						ZDLibraryLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ZDLibrary | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check discord for help with ZDLibrary specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= ZDLibraryLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					ZDLibraryLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			ZDLibrary.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("ZDLibrary | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check discord for help with ZDLibrary specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					ZDLibraryLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("ZDLibrary | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check discord for help with ZDLibrary specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					ZDLibraryLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("ZDLibrary | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check discord for help with ZDLibrary specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					ZDLibraryLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= ZDLibraryLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ZDLibrary | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check discord for help with ZDLibrary specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						ZDLibraryLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("ZDLibrary | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check discord for help with ZDLibrary specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					ZDLibraryLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			ZDLibraryLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			ZDLibraryLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('ZDLibrary had an issue creating settings.') end
...
function ZDLibraryLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function ZDLibraryLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function ZDLibraryLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['ZDLibrary Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.ZDLibraryOpen.Value or 'P'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function ZDLibraryLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				ZDLibraryLibrary:Notify({Title = "ZDLibrary Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			ZDLibraryLibrary:Notify({Title = "ZDLibrary Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('ZDLibrary Configurations Error | '..tostring(result))
			ZDLibraryLibrary:Notify({Title = "ZDLibrary Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = ZDLibraryLibrary:CreateWindow({
		Name = "ZDLibrary Example Window",
		LoadingTitle = "ZDLibrary Interface Suite",
		Theme = 'Default',
		Icon = 0,
		LoadingSubtitle = "by ZD",
		ConfigurationSaving = {
			Enabled = true,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using ZDLibrary may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like ZDLibrary to get the key from
	...
	--ZDLibraryLibrary:Notify({Title = "ZDLibrary Interface", Content = "Welcome to ZDLibrary. These - are the brand new notification design for ZDLibrary, with custom sizing and ZDLibrary calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(ZDLibraryLibrary.Theme) do
...
task.delay(4, function()
	ZDLibraryLibrary.LoadConfiguration()
